...
static int performance_monitor(void*)
{
	auto x = SINGLETON(TaskQueue);
				...
				for (bool i = true; ; i = false)
				{
					...
					if (WAIT_OBJECT_0 == dwWaitResult)
					{
						...
						for (int n = 0; n < nCount; n++)
						{
							...
							x->addCPUMetric(100 * cvProcessor.doubleValue / (cvIdle.doubleValue+ cvPriveleged.doubleValue+ cvProcessor.doubleValue), n);
